  ...
  ucdrBuffer * cdr)
{
  ...
  rv = ucdr_serialize_string(cdr, ros_message->measurement_source_name.data);
  if (rv) {
    ros_message->measurement_source_name.size = strlen(ros_message->measurement_source_name.data);
  }
  // Member: metrics_source
  rv = ucdr_serialize_string(cdr, ros_message->metrics_source.data);
  if (rv) {
    ros_message->metrics_source.size = strlen(ros_message->metrics_source.data);
  }
  // Member: unit
  rv = ucdr_serialize_string(cdr, ros_message->unit.data);
  if (rv) {
    ros_message->unit.size = strlen(ros_message->unit.data);
  }
  // Member: window_start
  ...
  void * untyped_ros_message)
{
  ...
  // Field name: measurement_source_name
  {
    ...
    rv = ucdr_deserialize_string(cdr, ros_message->measurement_source_name.data, capacity);
    if (rv) {
      ros_message->measurement_source_name.size = strlen(ros_message->measurement_source_name.data);
    }
  }
  // Field name: metrics_source
  {
    ...
    rv = ucdr_deserialize_string(cdr, ros_message->metrics_source.data, capacity);
    if (rv) {
      ros_message->metrics_source.size = strlen(ros_message->metrics_source.data);
    }
  }
  // Field name: unit
  {
    ...
    rv = ucdr_deserialize_string(cdr, ros_message->unit.data, capacity);
    if (rv) {
      ros_message->unit.size = strlen(ros_message->unit.data);
    }
  }
  ...
  // Member: statistics
  {
    ...
    for(size_t i = 0; i < sequence_size; i++){
      size_t element_size = ((const message_type_support_callbacks_t *)(
        ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_microxrcedds_c, statistics_msgs, msg, StatisticDataPoint
        )()->data))->get_serialized_size(&ros_message->statistics.data[i]);
