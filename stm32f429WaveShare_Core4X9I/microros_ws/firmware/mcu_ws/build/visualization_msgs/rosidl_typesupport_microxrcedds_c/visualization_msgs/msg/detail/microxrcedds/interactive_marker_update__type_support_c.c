  ...
  ucdrBuffer * cdr)
{
  ...
  // Member: server_id
  rv = ucdr_serialize_string(cdr, ros_message->server_id.data);
  if (rv) {
    ros_message->server_id.size = strlen(ros_message->server_id.data);
  }
  // Member: seq_num
  ...
  // Member: erases
  {
    ...
    for (size_t i = 0; rv && i < size; ++i) {
      rv = ucdr_serialize_string(cdr, ros_message->erases.data[i].data);
    }
  }
  ...
  void * untyped_ros_message)
{
  ...
  // Field name: server_id
  {
    ...
    rv = ucdr_deserialize_string(cdr, ros_message->server_id.data, capacity);
    if (rv) {
      ros_message->server_id.size = strlen(ros_message->server_id.data);
    }
  }
  ...
  // Field name: erases
  {
    ...
    for (size_t i = 0; rv && i < size; i++) {
      ...
      if (rv) {
        ros_message->erases.data[i].size = strlen(data);
      }
    }
  }
  return rv;
  ...
  size_t current_alignment)
{
  ...
  // Member: markers
  {
    ...
    for(size_t i = 0; i < sequence_size; i++){
      size_t element_size = ((const message_type_support_callbacks_t *)(
        ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_microxrcedds_c, visualization_msgs, msg, InteractiveMarker
        )()->data))->get_serialized_size(&ros_message->markers.data[i]);
  ...
  // Member: poses
  {
    ...
    for(size_t i = 0; i < sequence_size; i++){
      size_t element_size = ((const message_type_support_callbacks_t *)(
        ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_microxrcedds_c, visualization_msgs, msg, InteractiveMarkerPose
        )()->data))->get_serialized_size(&ros_message->poses.data[i]);
      uint8_t alignment_size = (element_size < MICROXRCEDDS_PADDING) ? element_size : MICROXRCEDDS_PADDING;
      current_alignment += ucdr_alignment(current_alignment, alignment_size) + element_size;
    }
  }
