  ...
  ucdrBuffer * cdr)
{
  ...
  // Member: source
  rv = ucdr_serialize_string(cdr, ros_message->source.data);
  if (rv) {
    ros_message->source.size = strlen(ros_message->source.data);
  }
  ...
  void * untyped_ros_message)
{
  ...
  // Field name: source
  {
    ...
    rv = ucdr_deserialize_string(cdr, ros_message->source.data, capacity);
    if (rv) {
      ros_message->source.size = strlen(ros_message->source.data);
    }
  }
