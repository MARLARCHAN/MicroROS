  ...
  ucdrBuffer * cdr)
{
  ...
  // Member: id
  rv = ucdr_serialize_string(cdr, ros_message->id.data);
  if (rv) {
    ros_message->id.size = strlen(ros_message->id.data);
  }
  // Member: passed
  ...
  void * untyped_ros_message)
{
  ...
  // Field name: id
  {
    ...
    rv = ucdr_deserialize_string(cdr, ros_message->id.data, capacity);
    if (rv) {
      ros_message->id.size = strlen(ros_message->id.data);
    }
  }
  ...
  // Member: status
  {
    ...
    for(size_t i = 0; i < sequence_size; i++){
      size_t element_size = ((const message_type_support_callbacks_t *)(
        ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_microxrcedds_c, diagnostic_msgs, msg, DiagnosticStatus
        )()->data))->get_serialized_size(&ros_message->status.data[i]);
