  ...
  ucdrBuffer * cdr)
{
  ...
  // Member: reason
  rv = ucdr_serialize_string(cdr, ros_message->reason.data);
  if (rv) {
    ros_message->reason.size = strlen(ros_message->reason.data);
  }
  ...
  void * untyped_ros_message)
{
  ...
  // Field name: reason
  {
    ...
    rv = ucdr_deserialize_string(cdr, ros_message->reason.data, capacity);
    if (rv) {
      ros_message->reason.size = strlen(ros_message->reason.data);
    }
  }
