...
#include "example_interfaces/msg/detail/string__rosidl_typesupport_microxrcedds_c.h"
...
typedef example_interfaces__msg__String _String__ros_msg_type;
  ...
  ucdrBuffer * cdr)
{
  ...
  // Member: data
  rv = ucdr_serialize_string(cdr, ros_message->data.data);
  if (rv) {
    ros_message->data.size = strlen(ros_message->data.data);
  }
  ...
  void * untyped_ros_message)
{
  ...
  // Field name: data
  {
    ...
    rv = ucdr_deserialize_string(cdr, ros_message->data.data, capacity);
    if (rv) {
      ros_message->data.size = strlen(ros_message->data.data);
    }
  }
...
{
  bool full_bounded;
  return max_serialized_size_example_interfaces__msg__String(&full_bounded, 0);
}
