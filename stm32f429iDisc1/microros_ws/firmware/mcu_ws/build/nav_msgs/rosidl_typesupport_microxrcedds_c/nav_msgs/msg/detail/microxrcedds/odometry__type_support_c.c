  ...
  ucdrBuffer * cdr)
{
  ...
  rv = ucdr_serialize_string(cdr, ros_message->child_frame_id.data);
  if (rv) {
    ros_message->child_frame_id.size = strlen(ros_message->child_frame_id.data);
  }
  // Member: pose
  rv = ((const message_type_support_callbacks_t *)(
      ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_microxrcedds_c, geometry_msgs, msg, PoseWithCovariance
  ...
  void * untyped_ros_message)
{
  ...
  // Field name: child_frame_id
  {
    ...
    rv = ucdr_deserialize_string(cdr, ros_message->child_frame_id.data, capacity);
    if (rv) {
      ros_message->child_frame_id.size = strlen(ros_message->child_frame_id.data);
    }
  }
