  ...
  ucdrBuffer * cdr)
{
  ...
  // Member: encoding
  rv = ucdr_serialize_string(cdr, ros_message->encoding.data);
  if (rv) {
    ros_message->encoding.size = strlen(ros_message->encoding.data);
  }
  // Member: is_bigendian
  ...
  void * untyped_ros_message)
{
  ...
  // Field name: encoding
  {
    ...
    rv = ucdr_deserialize_string(cdr, ros_message->encoding.data, capacity);
    if (rv) {
      ros_message->encoding.size = strlen(ros_message->encoding.data);
    }
  }
