  ...
  ucdrBuffer * cdr)
{
  ...
  // Member: names
  {
    ...
    for (size_t i = 0; rv && i < size; ++i) {
      rv = ucdr_serialize_string(cdr, ros_message->names.data[i].data);
    }
  }
  // Member: prefixes
  {
    ...
    rv = ucdr_serialize_uint32_t(cdr, size);
    for (size_t i = 0; rv && i < size; ++i) {
      rv = ucdr_serialize_string(cdr, ros_message->prefixes.data[i].data);
    }
  }
  ...
  void * untyped_ros_message)
{
  ...
  // Field name: names
  {
    ...
    for (size_t i = 0; rv && i < size; i++) {
      ...
      if (rv) {
        ros_message->names.data[i].size = strlen(data);
      }
    }
  }
  // Field name: prefixes
  {
    ...
    for (size_t i = 0; rv && i < size; i++) {
      ...
      rv = ucdr_deserialize_string(cdr, data, capacity);
      if (rv) {
        ros_message->prefixes.data[i].size = strlen(data);
      }
    }
  }
  return rv;
