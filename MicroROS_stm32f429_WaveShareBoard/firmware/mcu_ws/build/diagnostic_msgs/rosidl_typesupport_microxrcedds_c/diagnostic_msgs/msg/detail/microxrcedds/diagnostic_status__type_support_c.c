  ...
  ucdrBuffer * cdr)
{
  ...
  // Member: hardware_id
  rv = ucdr_serialize_string(cdr, ros_message->hardware_id.data);
  if (rv) {
    ros_message->hardware_id.size = strlen(ros_message->hardware_id.data);
  }
  // Member: values
  {
  ...
  void * untyped_ros_message)
{
  ...
  // Field name: hardware_id
  {
    ...
    rv = ucdr_deserialize_string(cdr, ros_message->hardware_id.data, capacity);
    if (rv) {
      ros_message->hardware_id.size = strlen(ros_message->hardware_id.data);
    }
  }
  ...
  // Member: values
  {
    ...
    for(size_t i = 0; i < sequence_size; i++){
      size_t element_size = ((const message_type_support_callbacks_t *)(
        ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_microxrcedds_c, diagnostic_msgs, msg, KeyValue
        )()->data))->get_serialized_size(&ros_message->values.data[i]);
