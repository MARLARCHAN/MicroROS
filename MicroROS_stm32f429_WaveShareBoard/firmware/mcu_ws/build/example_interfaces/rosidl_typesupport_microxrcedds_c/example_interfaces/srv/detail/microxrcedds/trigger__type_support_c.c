...
#include "example_interfaces/srv/detail/trigger__rosidl_typesupport_microxrcedds_c.h"
...
typedef example_interfaces__srv__Trigger_Request _Trigger_Request__ros_msg_type;
...
{
  bool full_bounded;
  return max_serialized_size_example_interfaces__srv__Trigger_Request(&full_bounded, 0);
}
...
typedef example_interfaces__srv__Trigger_Response _Trigger_Response__ros_msg_type;
  ...
  ucdrBuffer * cdr)
{
  ...
  // Member: message
  rv = ucdr_serialize_string(cdr, ros_message->message.data);
  if (rv) {
    ros_message->message.size = strlen(ros_message->message.data);
  }
  ...
  void * untyped_ros_message)
{
  ...
  // Field name: message
  {
    ...
    rv = ucdr_deserialize_string(cdr, ros_message->message.data, capacity);
    if (rv) {
      ros_message->message.size = strlen(ros_message->message.data);
    }
  }
...
{
  bool full_bounded;
  return max_serialized_size_example_interfaces__srv__Trigger_Response(&full_bounded, 0);
}
